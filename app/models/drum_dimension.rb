class DrumDimension < ActiveRecord::Base
  belongs_to :item
  ...
  def self.get_csv_headers
    self.csv_columns.collect {|column| "drum_dimension:" + column }
  end

  def get_csv_row
   DrumDimension.csv_columns.collect {|column| csv_safe(self.try(column.to_sym)) }
  end
