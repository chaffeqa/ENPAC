class CircularDimension < ActiveRecord::Base
  belongs_to :item
  ...
  def self.get_csv_headers
    self.csv_columns.collect {|column| "circular_dimension:" + column }
  end

  def get_csv_row
   CircularDimension.csv_columns.collect {|column| csv_safe(self.try(column.to_sym)) }
  end
